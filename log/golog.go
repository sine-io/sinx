...
// GologWrapper creates a new golog.Logger that uses zerolog for output.
...
func GologWrapper(logger *zerolog.Logger) *golog.Logger {
	return golog.New(
		logger.Hook(
			zerolog.HookFunc(func(e *zerolog.Event, level zerolog.Level, msg string) {
				/*
					e.g: "[INFO] serf: EventMemberJoin: node1 192.168.0.103"
				*/
