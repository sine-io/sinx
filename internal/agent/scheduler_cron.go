...
func (s *CronScheduler) Start(jobs []*Job) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *CronScheduler) Restart(jobs []*Job) {
...
func (s *CronScheduler) GetCronEntryJob(jobName string) (CronEntryJob, bool) {
	for _, e := range s.Cron.Entries() {
		if j, ok := e.Job.(*Job); !ok {
			s.logger.Error().
	...
	return CronEntryJob{}, false
}

// AddJob Adds a job to the cron scheduler
func (s *CronScheduler) AddJob(job *Job) error {
